...
public class listaPagos extends RecyclerView.Adapter<listaPagos.listaPagosHolder> {
    ...
    public void onBindViewHolder(@NonNull listaPagosHolder holder, int position) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            fecha = LocalDate.parse(pagos.getFechaVencimiento().toString());
        }
        ...
        holder.txtCosto.setText("S/." + pagos.getMontoCuo());
        holder.txtFechaDeuda.setText("Deuda del " + fechaFormateada2 + " al " + fechaFormateada3);


        holder.txtEstado.setText(pagos.getEstadoCuota());

        // Aplicar el color del texto basado en el estado de la cuota
        if (pagos.getEstadoCuota().equals("Pagado")) {
            holder.txtEstado.setTextColor(ContextCompat.getColor(context, R.color.ColorPrincipal2));
        } else {
            // Si el estado no es "Pagado", aplicar el color predeterminado (o el que desees)
            holder.txtEstado.setTextColor(ContextCompat.getColor(context, R.color.colorMorosidad));
        }
